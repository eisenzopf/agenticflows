...
// TextProcessor handles text generation and attribute extraction
type TextProcessor struct {
	analyzer *core.Analyzer
}

// NewTextProcessor creates a new TextProcessor instance
func NewTextProcessor(analyzer *core.Analyzer) *TextProcessor {
...
func (t *TextProcessor) GenerateRequiredAttributes(
	ctx context.Context,
	questions []string,
	existingAttributes []string,
) ([]models.AttributeDefinition, error) {
	// Validate request
	if len(questions) == 0 {
...
Return a JSON object with this structure:
{
  "attributes": [
	...
	attributes := make([]models.AttributeDefinition, 0, len(attributesRaw))
...
func (t *TextProcessor) GenerateAttribute(
	ctx context.Context,
	text string,
	attribute models.AttributeDefinition,
) (*models.AttributeValue, error) {
	// Validate inputs
	if text == "" {
...
func (t *TextProcessor) GenerateAttributes(
	ctx context.Context,
	text string,
	...
	// Validate inputs
	if text == "" {
		emptyValues := make([]models.AttributeValue, 0, len(attributes))
		for _, attr := range attributes {
			emptyValues = append(emptyValues, models.AttributeValue{
...
Include all requested attributes in your response, even if the confidence is low.`,
	...
	attrValues := make([]models.AttributeValue, 0, len(attrValuesRaw))
...
func (t *TextProcessor) GenerateIntent(ctx context.Context, text string) (*models.IntentClassification, error) {
	// Validate input
	if text == "" {
...
%s`, truncateText(text, 8000))
	...
	intent := &models.IntentClassification{
