...
// PlannerProcessor creates action plans based on analysis and recommendations
type PlannerProcessor struct {
	analyzer *core.Analyzer
}

// NewPlannerProcessor creates a new PlannerProcessor instance
func NewPlannerProcessor(analyzer *core.Analyzer) *PlannerProcessor {
	return &PlannerProcessor{
		analyzer: analyzer,
	}
}

// CreateActionPlan creates an implementation plan based on recommendations
func (p *PlannerProcessor) CreateActionPlan(
	ctx context.Context,
	recommendations *models.RecommendationResponse,
	constraints map[string]interface{},
) (*models.ActionPlan, error) {
	// Validate input
...
func (p *PlannerProcessor) GenerateTimeline(
	ctx context.Context,
...
[
  {
	...
	result, err := p.analyzer.LLMClient.GenerateContent(ctx, prompt, []interface{}{})
	if err != nil {
		return nil, fmt.Errorf("failed to generate content: %w", err)
	}
	...
	timeline := make([]models.TimelineEvent, 0, len(resultArray))
...
func (p *PlannerProcessor) extractActionItems(resultMap map[string]interface{}, key string) []models.ActionItem {
